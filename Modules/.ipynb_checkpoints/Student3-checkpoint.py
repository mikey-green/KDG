'''
Student LightGCN model for Knowledge Distillation
匹配Teacher模型（LightGCN）的核心逻辑，仅调整嵌入维度等可配置参数
'''
import torch
import torch.nn as nn

# 复用Teacher的GraphConv类（结构完全一致）
class GraphConv(nn.Module):
    """
    Graph Convolutional Network (和Teacher完全一致)
    """
    def __init__(self, n_hops, n_users, interact_mat,
        ...
        self.dropout = nn.Dropout(p=0.2)  # 提高Dropout比例，增强正则化效果
    ...
    def forward(self, user_embed, item_embed,
        ...
        for hop in range(self.n_hops):
            ...
            # 训练时强制启用Dropout，增强正则化
            agg_embed = self.dropout(agg_embed)
            embs.append(agg_embed)
        embs = torch.stack(embs, dim=1)  # [n_entity, n_hops+1, emb_size]
        return embs[:self.n_users, :], embs[self.n_users:, :]

class StudentLightGCN(nn.Module):
    def __init__(self, data_config, args_config, adj_mat):
        super(StudentLightGCN, self).__init__()
        # 基础配置（和Teacher一致）
        ...
        if self.args.gpu_id < 0 or not torch.cuda.is_available():
            self.device = torch.device('cpu')
            print("[StudentLightGCN] Using CPU (no CUDA or gpu_id < 0).")
        else:
            self.device = torch.device(f'cuda:{self.args.gpu_id}')
            print(f"[StudentLightGCN] Using GPU: cuda:{self.args.gpu_id}")
        # 初始化权重（嵌入维度为Student的dim）
        self._init_weight()
        self.user_embed = nn.Parameter(self.user_embed)
        self.item_embed = nn.Parameter(self.item_embed)
        # 初始化GCN层（使用Student的传播层数）
        self.gcn = self._init_model()
########################################################################             
        # ===== Projection layer for KD (32 -> 64) =====
        self.kd_proj = nn.Linear(self.emb_size, 64)
        # 初始化投影层权重（避免训练初期梯度爆炸）
        nn.init.xavier_uniform_(self.kd_proj.weight)
        nn.init.zeros_(self.kd_proj.bias)
########################################################################
        
    def _init_weight(self):
        """初始化用户/物品嵌入（维度为Student的emb_size）"""
        initializer = nn.init.xavier_uniform_
        self.user_embed = initializer(torch.empty(self.n_users, self.emb_size))
        self.item_embed = initializer(torch.empty(self.n_items, self.emb_size))
        # 转换邻接矩阵为稀疏张量（和Teacher一致）
...
#新增代码
    def generate(self, split=True, return_layers=False):
        """
        return_layers=True 时返回每一层embedding用于蒸馏
        """
        user_gcn_emb, item_gcn_emb = self.gcn(self.user_embed,
                                          self.item_embed,
                                          edge_dropout=False,
