...
##############################################################################
    ...
    def init_logger(args):
        ...
        log_file = f"student_{args.dataset}_dim{args.dim}.txt"
...
####################################################################################################
    ...
    for epoch in range(args.epoch):
        ...
        alpha = 2.0   # 先用2，别用4
        #time_weight = math.exp(-alpha * (1 - progress))
...
####################################################################################################         
            ...
            # ===== KD Loss =====
            #kd_loss = F.mse_loss(s_pos_scores, t_pos_scores) + \
            #         F.mse_loss(s_neg_scores, t_neg_scores)
            ...
            #score_kd = kl_div_with_logits(s_logits, t_logits, temperature=4.0)
            ...
            # 计算 batch 内 user-item 相似度
            #sim_s = torch.sum(s_users_proj * s_items_proj, dim=1)
            #sim_t = torch.sum(t_users_batch * t_items_batch, dim=1)
            
            # 原有 user-item 相似度
            #sim_s_ui = torch.sum(s_users_proj * s_items_proj, dim=1)
            #sim_t_ui = torch.sum(t_users_batch * t_items_batch, dim=1)
            ...
            #struct_ui = F.mse_loss(sim_s_ui, sim_t_ui)
            
            struct_kd = F.mse_loss(sim_s_ui, sim_t_ui)
            #归一化
            #struct_kd = struct_kd / users.shape[0]
            
            
            # user-user 相似度矩阵（batch 内）
            #sim_s_uu = torch.matmul(s_users_proj, s_users_proj.T)
            #sim_t_uu = torch.matmul(t_users_batch, t_users_batch.T)
            #struct_uu = F.mse_loss(sim_s_uu, sim_t_uu)
            
            # item-item 相似度矩阵
            #sim_s_ii = torch.matmul(s_items_proj, s_items_proj.T)
            #sim_t_ii = torch.matmul(t_items_batch, t_items_batch.T)
            #struct_ii = F.mse_loss(sim_s_ii, sim_t_ii)
            
            # 加权合并
            #struct_kd = struct_ui + 0.5 * (struct_uu + struct_ii)
            #struct_kd = struct_ui

            #用户两两关系蒸馏
            #sim_s = torch.matmul(s_users_proj, s_users_proj.t())
            #sim_t = torch.matmul(t_users_batch, t_users_batch.t())
            #struct_kd = F.mse_loss(sim_s, sim_t)
...
####################################################################################################
        ...
        if batch_count > 0:
            '''
            print("KD avg:",
                "score=%.6f" % (score_sum / batch_count),
                "rep=%.6f" % (rep_sum / batch_count),
                "struct=%.6f" % (struct_sum / batch_count))
            '''
            logger.info(
...
#################################################################################################### 
            ...
            if valid_ret['recall'][0] == cur_best_pre_0:
                ...
                #print(f"Best Student saved to {save_path}")
